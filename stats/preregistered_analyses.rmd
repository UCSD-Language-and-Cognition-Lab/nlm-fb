...
ggsave("../Figures/log_odds_9.pdf", dpi = 300, width=9, height=5)
```
...
df_merged_summ %>%
  mutate("GPT-3" = p_start_gpt3,
         "Human" = prop_start) %>%
  pivot_longer(cols = c("Human", "GPT-3"),
               names_to = "metric",
               values_to = "value") %>%
  ggplot(aes(x = value,
             fill = condition)) +
...
ggsave("../Figures/comparison_pstart.pdf", dpi = 300, width=7, height=5)
